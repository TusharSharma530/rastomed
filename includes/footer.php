...
if (!isset($footerdesc)) {
    ...
    if ($stmt) {
        ...
        $settingsResult = mysqli_stmt_get_result($stmt);
        if ($settingsResult && mysqli_num_rows($settingsResult) > 0) {
            $rw = mysqli_fetch_assoc($settingsResult);
            $footerdesc =$rw['footerdesc'] ?? '';
            $address =$rw['address'] ?? '';
            $contactno =$rw['contact_no'] ?? '';
            $alternateno =$rw['alternate_no'] ?? '';
            $emailid =$rw['email_id'] ?? '';
            $whatsapp =$rw['whatsapp_no'] ?? '';
    ...
    // Fallback defaults if query fails or returns empty
  ...
  <div class="container">
    ...
    <div class="footer__main">
      ...
      <div class="footer__brand">
        <a href="index.php" class="footer__logo" aria-label="<?= htmlspecialchars($websitename) ?> Home">
          <?php if(!empty($logo)): ?>
        ...
        <div class="footer__social">
          ...
          <a href="<?= htmlspecialchars($linkedin) ?>" target="_blank" class="footer__social-link" aria-label="LinkedIn">
            <img src="assets/images/linkedin.png" alt="LinkedIn" class="footer-icon-img">
          </a>
          <?php endif; ?>
          <?php if(!empty($twitter)): ?>
          <a href="<?= htmlspecialchars($twitter) ?>" target="_blank" class="footer__social-link" aria-label="X (Twitter)">
            <img src="assets/images/x-twitter.svg" alt="X" class="footer-icon-img footer-icon-img--x">
          </a>
          <?php endif; ?>
          <?php if(!empty($instagram)): ?>
          <a href="<?= htmlspecialchars($instagram) ?>" target="_blank" class="footer__social-link" aria-label="Instagram">
      ...
      <div class="footer__column">
        ...
        <div class="footer__contact-item">
          ...
          <span>+91 <?= htmlspecialchars($contactno) ?><?php if(!empty($alternateno)): ?><br>+91 <?= htmlspecialchars($alternateno) ?><?php endif; ?></span>
        </div>
        <?php endif; ?>
        ...
        <div class="footer__contact-item">
          ...
          <span><?= htmlspecialchars($emailid) ?></span>
        </div>
        <?php endif; ?>
      </div>
    </div>
...
<div class="enquiry-float">
  ...
  <a href="https://wa.me/91<?= htmlspecialchars($whatsapp) ?>" target="_blank" class="enquiry-float__btn whatsapp-float__btn" aria-label="Chat on WhatsApp">
