<?php include 'config.php';
	...
	if(isset($_POST['deletedata'])){
		$id = $_POST['id'];
		$sqldelImg = mysqli_query($con, "SELECT * FROM toppers WHERE id = $id");
			if(mysqli_num_rows($sqldelImg)){
				$rowimg = mysqli_fetch_assoc($sqldelImg);
					$imgid = $rowimg['image'];
					unlink("../".$imgid);

				$sqldelete = mysqli_query($con,"DELETE FROM toppers WHERE id = $id");
				if($sqldelete){
					echo 'true';
				}else{
					echo 'false';
...
<div class="col-md-12">
	<div class="page-content">
		...
		<table class="table table-hover" id="myTable">
			...
			<tbody>
<?php $sqlgallery  = mysqli_query($con, "SELECT * FROM `toppers` ORDER BY `order` DESC");
...
?>
				<tr id='remove<?php echo $id; ?>'>
					...
					<td><?=$rwgallery['subject'];?></td>
					<td><?=$rwgallery['order'];?></td>
					<td>
						<div class="form-check form-switch">
							<?php $checked = $rwgallery['status']==1 ? "checked" : ""; ?>
						  <input class="form-check-input" type="checkbox" data-table="toppers" ide="<?=$rwgallery['id'];?>" <?=$checked;?>>
						  <label class="form-check-label" for="status"></label>
						</div>
					</td>
					<td class="text-end">
						<a href="updatetopper.php?id=<?=$id;?>" class="editbtn ri-pencil-line" ></a> 
