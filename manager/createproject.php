...
if(isset($_POST['addTc'])){
	...
	$sqlcheck = mysqli_query($con,"SELECT * FROM projects WHERE title = '$title'");
	if(mysqli_num_rows($sqlcheck)){
		echo "<script>swal('Already in Record', 'Click `OK` to try Again', 'warning'); $('#submitForm').show();  </script>";
		
	}else{
		$sqlins = mysqli_query($con,"INSERT INTO projects (id, title, url, `desc`, `sdesc`, file, `order`) VALUES (NULL, '$title', '$url', '$desc', '$sdesc', '$uploadpath', '$order')");
		
	if($sqlins){
		echo "<script>swal('Added Successfully', 'Click `OK` to Close', 'success'); 
				$('#submitForm').hide();
			 </script>";
		echo "<div class='col-md-12 padd0 text-center'><a href='createproject.php' class=' btn btn-primary'>Create New</a></div>";
	}else{
