...
if(!isset($_SESSION['username'])){
	...
	if(isset($_POST['deletedata'])){
		$id = $_POST['id'];
		$sqldelete = mysqli_query($con,"DELETE FROM enquiry WHERE id = {$id}");
		if($sqldelete){
			echo 'true';
		}else{
			echo 'false';
...
<div class="col-md-12">
	...
	<table class="table table-hover" id="myTable">
		...
		<?php 
			...
			$sqlcontact  = mysqli_query($con, "SELECT * FROM enquiry ORDER BY id DESC");
		 ...
		 ?>
			<tr id='remove<?php echo $cid; ?>'>
				...
				<td><?php echo $rwcontact['father_name']; ?></td>
				<td><?=$rwcontact['phone'];?></td>		
				<td><?=$rwcontact['class'];?></td>		
