...
$id = $_POST['id'];
$sqldelImg = mysqli_query($con, "SELECT * FROM quickaccess WHERE id = $id");
	if(mysqli_num_rows($sqldelImg)){
		$rowimg = mysqli_fetch_assoc($sqldelImg);
			$imgid = $rowimg['file'];
			unlink("../".$imgid);

		$sqldelete = mysqli_query($con,"DELETE FROM quickaccess WHERE id = $id");
		if($sqldelete){
			echo 'true';
		}else{
			echo 'false';
	...
	<div class="page-content">
		...
		<table class="table table-hover" id="myTable">
			...
			<?php 
				...
				$sqltc  = mysqli_query($con, "SELECT * FROM quickaccess ORDER BY id ASC");
			 ...
			 ?>
				<tr id='remove<?php echo $tcid; ?>'>
					...
					<div class="form-check form-switch">
						  ...
						  <input class="form-check-input" type="checkbox" data-table="quickaccess" ide="<?=$rwtc['id'];?>" <?=$checked;?>>
						  <label class="form-check-label" for="status"></label>
						</div>
					</td>
					<td class="text-center">
						<a href="updatequickaccess.php?id=<?=$tcid; ?>" class="editbtn ri-pencil-line" ></a> 
