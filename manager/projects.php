<?php include 'config.php';
	...
	if(isset($_POST['deletedata'])){
		$id = $_POST['id'];
		$sqldelImg = mysqli_query($con, "SELECT * FROM projects WHERE id = {$id}");
			if(mysqli_num_rows($sqldelImg)){
				$rowimg = mysqli_fetch_assoc($sqldelImg);
					$imgid = $rowimg['file'];
					unlink("../".$imgid);

				$sqldelete = mysqli_query($con,"DELETE FROM projects WHERE id = {$id}");
				if($sqldelete){
					echo 'true';
				}else{
					echo 'false';
	...
	<div class="page-content">
		...
		<table class="table table-hover" id="myTable">
			...
			<?php 
				...
				$sqltc  = mysqli_query($con, "SELECT * FROM projects ORDER BY id ASC");
			 ...
			 ?>
				<tr id='remove<?php echo $tcid; ?>'>
					...
					<td><?=$rwtc['order'];?></td>
					<td class="text-end">
						<a href="updateproject.php?tcupid=<?php echo $tcid; ?>"><i class='bx bx-edit mx-2 text-success'></i></a> 
