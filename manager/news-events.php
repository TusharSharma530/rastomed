...
if(isset($_POST['deletedata'])){
	$id = $_POST['id'];
	$sqldelImg = mysqli_query($con, "SELECT * FROM `news_events` WHERE id = {$id}");
		if(mysqli_num_rows($sqldelImg)){
			$rowimg = mysqli_fetch_assoc($sqldelImg);
				$imgid = $rowimg['file'];
				unlink("../".$imgid);

			$sqldelete = mysqli_query($con,"DELETE FROM `news_events` WHERE id = {$id}");
			if($sqldelete){
				echo 'true';
			}else{
				echo 'false';
	...
	<div class="page-content">
		...
		<table class="table table-hover" id="myTable">
			...
			<?php 
				...
				$sqlnews  = mysqli_query($con, "SELECT * FROM `news_events` ORDER BY id ASC");
				...
				<tr id='remove<?php echo $id; ?>'>
					...
					<td>
						<div class="form-check form-switch">
						  ...
						  <input class="form-check-input" type="checkbox" data-table="news_events" ide="<?=$id;?>" <?=$checked;?>>
						  <label class="form-check-label" for="status"></label>
						</div>
					</td>
					<td class="text-center">
						<a href="updatenews-events.php?id=<?=$id; ?>" class="editbtn ri-pencil-line" ></a> 
